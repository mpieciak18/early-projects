...
/* Find min, avg, max pop*/
SELECT
    name AS "country with lowest population", MIN(population) AS 'population'
FROM
    countries;
SELECT
    name AS "country with highest population", MAX(population) AS 'population'
FROM
    countries;
SELECT
...
FROM
    countries;
/* Find min, avg, max % 1 year change*/
SELECT
    name AS "country with highest % population decay", MIN(percent_one_year_change) AS 'change'
FROM
    countries;
SELECT
    name AS "country with highest % population growth", MAX(percent_one_year_change) AS 'change'
FROM
    countries;
SELECT
    AVG(percent_one_year_change) AS "average % population change"
FROM
    countries;
/* Find min, avg, max total 1 year change*/
SELECT
    name AS "country with highest total population decay", MIN(population_change) AS 'change'
FROM
    countries;
SELECT
    name AS "country with highest total population growth", MAX(population_change) AS 'change'
FROM
    countries;
SELECT
    ROUND(AVG(population_change)) AS "average total population change"
FROM
    countries;
/* Find min, avg, max total pop density*/
SELECT
    name AS "country with lowest population density", MIN(density_per_sq_km) AS 'people per sq. mile'
FROM
    countries;
SELECT
    name AS "country with highest population density", MAX(density_per_sq_km) AS 'people per sq. mile'
FROM
    countries;
SELECT
    ROUND(AVG(density_per_sq_km)) AS "average population density (people per sq. mile)"
FROM
    countries;
/* Find min, avg, max area*/
SELECT
    name AS "country with lowest area", MIN(area_sq_km) AS 'area in sq. km'
FROM
    countries;
SELECT
    name AS "country with highest area", MAX(area_sq_km) AS 'area in sq. km'
FROM
    countries;
SELECT
    ROUND(AVG(area_sq_km)) AS "average area in sq. km"
FROM
    countries;
/* Find min, avg, max net migration*/
SELECT
    name AS "country with lowest net migration", MIN(net_migrants) AS 'net migration'
FROM
    countries;
SELECT
    name AS "country with highest net migration", MAX(net_migrants) AS 'net migration'
FROM
    countries;
SELECT
    ROUND(AVG(net_migrants)) AS 'net migration'
FROM
    countries;
/* Find min, avg, max fertility rate*/
SELECT
    name AS "country with lowest fertility rate", MIN(fertility_rate) AS 'fertility rate'
FROM
    countries;
SELECT
    name AS "country with highest fertility rate", MAX(fertility_rate) AS 'fertility rate'
FROM
    countries;
SELECT
    ROUND(AVG(fertility_rate)) AS 'average fertility rate'
FROM
    countries;
/* Find min, avg, max median_age*/
SELECT
    name AS "country with lowest median age", MIN(median_age) AS 'median age'
FROM
    countries;
SELECT
    name AS "country with highest median age", MAX(median_age) AS 'median age'
FROM
    countries;
SELECT
    ROUND(AVG(median_age)) AS 'average median age'
FROM
    countries;
/* Find min, avg, max % of world population*/
SELECT
    name AS "country with lowest % of world population", MIN(percent_of_world_pop) AS '% of world population'
FROM
    countries;
SELECT
    name AS "country with highest % of world population", MAX(percent_of_world_pop) AS '% of world population'
FROM
    countries;
SELECT
    AVG(percent_of_world_pop) AS 'average % of world population'
FROM
    countries;