...
# distance. The other component is the reaction distance, which is the product
...
# Equalling E and W gives the braking distance: d1 = v*v / 2*mu*g where g is
# the gravity of Earth and m the vehicle's mass.

# We have v in km per hour, g as 9.81 m/s/s and in the following we suppose that
# the reaction time is constant and equal to 1 s. The coefficient mu is 
# dimensionless.
...
#     The second task is to calculate v in km per hour knowing d in meters and
#     mu with the supposition that the reaction time is still t = 1.
...
def dist(v, mu):
    v = v / 3.6
    react_dist = v
    brake_dist = (v**2) / (2 * mu * 9.81)
    total_dist = react_dist + brake_dist
    return total_dist
